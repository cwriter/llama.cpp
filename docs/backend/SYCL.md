...
| GGML_SYCL_IQ3_REORDER | 0 or 1 (default) | Reorder IQ3_S MoE expert weights into a per-expert structure-of-arrays layout at first use, so the quants of a row are contiguous instead of interleaved with their scales. Speeds up the mat-vec (decode) and the grouped dequant GEMM (prefill). Set to 0 to keep the canonical block layout. |
| GGML_SYCL_IQ4_NL_REORDER | 0 or 1 (default) | The same structure-of-arrays reorder for IQ4_NL MoE expert weights. IQ4_NL shares the Q4_0 block layout, so the reorder is the same byte permutation; only the dequant differs. Set to 0 to keep the canonical block layout. |
