...
// mark nodes that the backend will fuse away, so their dst is never reserved
// absorbed is a per-node array of the graph passed to reserve/alloc; NULL clears the plan
// the plan is copied, and applies to every later reserve and alloc until it is replaced
