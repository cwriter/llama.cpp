    ...
    if (ggml_sycl_fattn_onednn_binds_kv(K, V)) {
        ...
        K_ptr = stage_k((size_t) ggml_nelements(K));
        {
            ...
            if (ggml_sycl_kv_is_soa(K)) {
                ...
                ggml_sycl_kv_soa_to_fp16(K_data, K_ptr, K->ne[0], K->ne[1], K->ne[2], K->nb[1], K->nb[2], stream);
                ...
                // the view's own strides hold for every layout, a head-interleaved cache included
                const int64_t ts = (int64_t) ggml_type_size(K->type);
        ...
        V_ptr = stage_v((size_t) ggml_nelements(V));
        {
            ...
            if (ggml_sycl_kv_is_soa(V)) {
                ...
                ggml_sycl_kv_soa_to_fp16(V_data, V_ptr, V->ne[0], V->ne[1], V->ne[2], V->nb[1], V->nb[2], stream);
                ...
                // the view's own strides hold for every layout, a head-interleaved cache included
                const int64_t ts = (int64_t) ggml_type_size(V->type);
