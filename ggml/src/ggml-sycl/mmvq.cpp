...
static void reorder_mul_mat_vec_iq3_q8_1_sycl(const void * vx, const void * vy, float * dst, const int ncols,
                                              const int nrows, dpct::queue_ptr stream) {
    static_assert(type == GGML_TYPE_IQ3_XXS || type == GGML_TYPE_IQ3_S);
    GGML_ASSERT(ncols % QK_K == 0);
    constexpr size_t num_subgroups = WARP_SIZE;
    const int block_num_y = ceil_div(nrows, GGML_SYCL_MMV_Y * (int) num_subgroups);
    const sycl::range<3> block_nums(1, 1, block_num_y);
    ...
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> nd_item) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                             mul_mat_vec_q_reorder<reorder_vec_dot_q_sycl<type>>(vx, vy, dst, ncols, nrows, nd_item);
                         });
    });
}

template <ggml_type type, int ncols_dst>
static void reorder_mul_mat_vec_iq3_q8_1_sycl_ncols(const void * vx, const void * vy, float * dst, const int ncols,
                                                    const int nrows, const int stride_col_y_bytes,
    ...
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> nd_item) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                             mul_mat_vec_q_reorder_ncols<reorder_vec_dot_q_sycl<type>, ncols_dst>(
                                 vx, /*vgate=*/ nullptr, vy, dst, ncols, nrows, stride_col_y_bytes, stride_col_dst,
                                 /*glu_op=*/ GGML_GLU_OP_SWIGLU, nd_item);
                         });
    });
...
template <ggml_type type>
static void reorder_mul_mat_vec_iq3_q8_1_sycl_switch_ncols(const void * vx, const void * vy, float * dst,
                                                           const int ncols, const int nrows, const int ncols_dst,
                                                           const int stride_col_y_bytes, const int stride_col_dst,
                                                           dpct::queue_ptr stream) {
    switch (ncols_dst) {
        case 1: reorder_mul_mat_vec_iq3_q8_1_sycl<type>(vx, vy, dst, ncols, nrows, stream); break;
        case 2: reorder_mul_mat_vec_iq3_q8_1_sycl_ncols<type, 2>(vx, vy, dst, ncols, nrows, stride_col_y_bytes, stride_col_dst, stream); break;
        case 3: reorder_mul_mat_vec_iq3_q8_1_sycl_ncols<type, 3>(vx, vy, dst, ncols, nrows, stride_col_y_bytes, stride_col_dst, stream); break;
        case 4: reorder_mul_mat_vec_iq3_q8_1_sycl_ncols<type, 4>(vx, vy, dst, ncols, nrows, stride_col_y_bytes, stride_col_dst, stream); break;
        case 5: reorder_mul_mat_vec_iq3_q8_1_sycl_ncols<type, 5>(vx, vy, dst, ncols, nrows, stride_col_y_bytes, stride_col_dst, stream); break;
        case 6: reorder_mul_mat_vec_iq3_q8_1_sycl_ncols<type, 6>(vx, vy, dst, ncols, nrows, stride_col_y_bytes, stride_col_dst, stream); break;
        case 7: reorder_mul_mat_vec_iq3_q8_1_sycl_ncols<type, 7>(vx, vy, dst, ncols, nrows, stride_col_y_bytes, stride_col_dst, stream); break;
        case 8: reorder_mul_mat_vec_iq3_q8_1_sycl_ncols<type, 8>(vx, vy, dst, ncols, nrows, stride_col_y_bytes, stride_col_dst, stream); break;
        default: GGML_ABORT("unsupported ncols_dst=%d for reordered IQ3 MMVQ", ncols_dst);
    }
}
            ...
            case GGML_TYPE_IQ3_XXS:
                if ((ggml_tensor_extra_gpu *) dst->src[0]->extra &&
                    ...
                    if (i == 0 && src1_ncols > 1 && src1_ncols <= 8) {
                        const int stride_col_y_bytes = src1_padded_col_size * q8_1_ts / q8_1_bs;
                        const int stride_col_dst     = dst->ne[0];
                        GGML_SYCL_DEBUG("Calling reorder_mul_mat_vec_iq3_q8_1_sycl_switch_ncols ncols=%d\n", (int)src1_ncols);
                        reorder_mul_mat_vec_iq3_q8_1_sycl_switch_ncols<GGML_TYPE_IQ3_XXS>(
                            src0_dd_i, src1_ddq_i, dst_dd_i, ne00, row_diff,
                            src1_ncols, stride_col_y_bytes, stride_col_dst, stream);
                        return;
                    } else {
                        GGML_SYCL_DEBUG("Calling reorder_mul_mat_vec_iq3_q8_1_sycl IQ3_XXS\n");
                        reorder_mul_mat_vec_iq3_q8_1_sycl<GGML_TYPE_IQ3_XXS>(
            ...
            case GGML_TYPE_IQ3_S:
                if ((ggml_tensor_extra_gpu *) dst->src[0]->extra &&
                    ...
                    if (i == 0 && src1_ncols > 1 && src1_ncols <= 8) {
                        ...
                        const int stride_col_dst     = dst->ne[0];
                        GGML_SYCL_DEBUG("Calling reorder_mul_mat_vec_iq3_q8_1_sycl_switch_ncols ncols=%d\n", (int)src1_ncols);
                        reorder_mul_mat_vec_iq3_q8_1_sycl_switch_ncols<GGML_TYPE_IQ3_S>(
                            src0_dd_i, src1_ddq_i, dst_dd_i, ne00, row_diff,
                            src1_ncols, stride_col_y_bytes, stride_col_dst, stream);
                        return;
                    } else {
                        GGML_SYCL_DEBUG("Calling reorder_mul_mat_vec_iq3_q8_1_sycl IQ3_S\n");
                        reorder_mul_mat_vec_iq3_q8_1_sycl<GGML_TYPE_IQ3_S>(
                            src0_dd_i, src1_ddq_i_bs, dst_dd_i_bs, ne00, row_diff, stream);
                    }
                } else {
...
// MoE mat-vec with a GLU epilogue: one sub-group computes the gate and the up dot products for
...
static void mul_mat_vec_q_moe_reorder_glu(
    ...
    const size_t src1_row_stride,
    const size_t ids_token_stride, const size_t dst_token_stride,
    const size_t src1_token_stride, const ggml_glu_op glu_op,
    const sycl::nd_item<3> & item_ct1) {
    using block_type   = ggml_sycl_reordered::block_q_t<reorder_vec_dot_q_sycl::gtype>;
    using block_traits = typename block_type::traits;
    ...
    const char * vx_gate = (const char *) vx_gate_base + (size_t) i02 * expert_weight_stride;
    const char * vx_up   = (const char *) vx_up_base   + (size_t) i02 * expert_weight_stride;
    const char * vy  = (const char *) vy_base + (size_t) token_idx * src1_token_stride + (size_t) expert_idx * src1_row_stride;
    float *      dst = (float *) ((char *) dst_base + (size_t) token_idx * dst_token_stride + (size_t) expert_idx * dst_row_stride);
...
template <typename reorder_vec_dot_q_sycl>
static void launch_mul_mat_vec_q_moe_reorder_glu(
    const void * vx_gate_base, const void * vx_up_base, const void * vy, const int32_t * ids_dev,
    float * dst_base, const int ncols, const int nrows, const int n_experts_used, const int n_tokens,
    const size_t expert_weight_stride, const size_t dst_row_stride,
    const size_t src1_row_stride, const size_t ids_token_stride,
    const size_t dst_token_stride, const size_t src1_token_stride,
...
bool ggml_sycl_mul_mat_vec_q_id_reorder_glu(
    ...
    switch (src0_type) {
        case GGML_TYPE_Q4_K:
            ...
            return true;
        case GGML_TYPE_Q5_K:
            launch_mul_mat_vec_q_moe_reorder_glu<reorder_vec_dot_q_sycl<GGML_TYPE_Q5_K>>(
                vx_gate_base, vx_up_base, vy, ids_dev, dst_base, ncols, nrows, n_experts_used, n_tokens,
                expert_weight_stride, dst_row_stride, src1_row_stride, ids_token_stride,
                dst_token_stride, src1_token_stride, glu_op, stream);
            return true;
        case GGML_TYPE_Q6_K:
            launch_mul_mat_vec_q_moe_reorder_glu<reorder_vec_dot_q_sycl<GGML_TYPE_Q6_K>>(
                vx_gate_base, vx_up_base, vy, ids_dev, dst_base, ncols, nrows, n_experts_used, n_tokens,
                expert_weight_stride, dst_row_stride, src1_row_stride, ids_token_stride,
                dst_token_stride, src1_token_stride, glu_op, stream);
            return true;
        case GGML_TYPE_IQ3_S:
            launch_mul_mat_vec_q_moe_reorder_glu<reorder_vec_dot_q_sycl<GGML_TYPE_IQ3_S>>(
                vx_gate_base, vx_up_base, vy, ids_dev, dst_base, ncols, nrows, n_experts_used, n_tokens,
                expert_weight_stride, dst_row_stride, src1_row_stride, ids_token_stride,
                dst_token_stride, src1_token_stride, glu_op, stream);
            return true;
        case GGML_TYPE_IQ4_NL:
            launch_mul_mat_vec_q_moe_reorder_glu<reorder_vec_dot_q_sycl<GGML_TYPE_IQ4_NL>>(
                vx_gate_base, vx_up_base, vy, ids_dev, dst_base, ncols, nrows, n_experts_used, n_tokens,
                expert_weight_stride, dst_row_stride, src1_row_stride, ids_token_stride,
                dst_token_stride, src1_token_stride, glu_op, stream);
            return true;
        case GGML_TYPE_Q8_0:
            launch_mul_mat_vec_q_moe_reorder_glu<reorder_vec_dot_q_sycl<GGML_TYPE_Q8_0>>(
                vx_gate_base, vx_up_base, vy, ids_dev, dst_base, ncols, nrows, n_experts_used, n_tokens,
                expert_weight_stride, dst_row_stride, src1_row_stride, ids_token_stride,
                dst_token_stride, src1_token_stride, glu_op, stream);
            return true;
