...
// Same dot product as reorder_vec_dot_q_sycl<GGML_TYPE_Q8_0>, but issuing one 16-byte load
// per operand instead of eight 2-byte loads (get_int_from_int8 splits every dword) plus four
// 4-byte ones. A lane's vdr_mmvq dwords are contiguous and q8_0's reorder layout places them
// at ibx*32 + iqs*4, so the base is 16-byte aligned and the sub-group issues one coalesced
// message per 256-byte run rather than gathering at a 16-byte stride.
...
// 0xff in byte j when bit j of signs is set. The grid bytes are never 0, so the
// per-byte negate can use a plain 32 bit add without carrying between lanes.
static __dpct_inline__ uint32_t iq_sign_mask(const uint32_t signs) {
    const uint32_t bits = (signs & 0x01) | ((signs & 0x02) << 7) | ((signs & 0x04) << 14) | ((signs & 0x08) << 21);

    return bits * 0xff;
}

// ksigns_iq2xs[i] without the table: the top bit is the parity of the low 7
...
template <> struct reorder_vec_dot_q_sycl<GGML_TYPE_IQ3_XXS> {
    ...
    __dpct_inline__ float operator()(const void * __restrict__ vbq, const std::pair<int, int> ibx_offset,
        ...
        const uint16_t * gas  = (const uint16_t *) (qs + QK_K / 4) + 2 * iqs;
...
#pragma unroll
        ...
        const ggml_half d = *(const ggml_half *) (base + d_offset.first);
...
template <> struct reorder_vec_dot_q_sycl<GGML_TYPE_IQ3_S> {
    ...
    __dpct_inline__ float operator()(const void * __restrict__ vbq, const std::pair<int, int> ibx_offset,
        ...
        const ggml_half d        = *(const ggml_half *) metadata;
