...
// straight into the final destination, so the wide product is never a graph tensor. The
// MUL_MAT, the RELU, the CONT and every ADD but the last are reported through
// fusion_absorbs, so ggml-alloc reserves nothing for them. Every test below is structural,
// so the answer is the same before and after allocation.
//
...
int ggml_sycl_fuse_qsa_score(ggml_backend_sycl_context & ctx, ggml_cgraph * cgraph, int i) {
    qsa_score_chain c;
    if (!g_ggml_sycl_enable_fusion || !g_ggml_sycl_fuse_qsa_score || !ggml_sycl_qsa_score_shape(cgraph, i, &c)) {
        return 0;
    }
    ...
    // GGML_SYCL_QSA_SCORE_TRACE gives the number of firings to report
    static std::atomic<int> trace_left{ getenv("GGML_SYCL_QSA_SCORE_TRACE") ?
                                        std::max(1, atoi(getenv("GGML_SYCL_QSA_SCORE_TRACE"))) : 0 };
    if (trace_left.fetch_sub(1) > 0) {
        fprintf(stderr, "[QSASCORE] k=%ld n_blocks=%ld n_heads=%ld n_tps=%ld n_stream=%ld t_tile=%ld dnn=%d\n",
                (long) k, (long) n_blocks, (long) n_heads, (long) n_tps, (long) n_stream, (long) t_tile,
                (int) (GGML_SYCL_DNNL && g_ggml_sycl_enable_dnn));
    }
