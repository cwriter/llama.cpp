...
// Rows with k at or below this go through the scan-merge kernels below, which hold
// (block + 1) * k (value, index) pairs in SLM and so cannot grow k. Above it a radix
// select whose SLM cost is independent of k takes over. Tune here only.
static constexpr int SYCL_TOP_K_SCAN_MERGE_MAX_K = 32;
...
static void top_k_radix_f32_sycl(
    ...
    // One group per row; every pass is a strided sweep of the row, so lanes in flight is
    // the lever, bounded by the device. Must cover the 256 buckets for the scan step.
    const int max_block = ggml_sycl_info().max_work_group_sizes[ctx.device];
    const int block_size = std::min(512, max_block);
...
static void ggml_sycl_op_top_k(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    if (k <= SYCL_TOP_K_SCAN_MERGE_MAX_K) {
        ...
        top_k_radix_f32_sycl(ctx, src0_dd, dst_dd, ncols, nrows, k, main_stream);
    }
}
